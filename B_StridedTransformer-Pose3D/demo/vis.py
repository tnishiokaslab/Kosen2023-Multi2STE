...
def show3Dpose(vals, ax):
    ...
    for i in np.arange( len(I) ):
        ...
        ax.plot(x, y, z, lw=2 )#,c='r') ###########################color=c  ######################
...
def get_pose3D(video_path, output_dir ,args ,video_name):
    txtdir = args.txtdir
    ...
    Path(output_dir+txtdir).mkdir(parents=True, exist_ok=True)
    ...
    for i in tqdm(range(video_length)):
        ...
        txt = (output_dir+txtdir + video_name + txtname + '.txt')
        ##txt書き込み#########################3333333
        if i ==  0:
            out_txt = post_out
        else:
            out_txt1 = np.loadtxt(txt)
            out_txt = np.vstack([out_txt1, post_out])
        np.savetxt(txt, out_txt)
        ##################################################
        """
        txt = (output_dir+txtdir + video_name +txtname + '_' + str(i) + '.txt')
        ##txt書き込み#########################3333333
        out_txt = post_out
        np.savetxt(txt, out_txt)
        ##################################################
    ...
    for i in tqdm(range(len(image_2d_dir))):
        ...
        output_dir_pose = output_dir +'pose/'
        os.makedirs(output_dir_pose, exist_ok=True)
        cv2.imwrite(output_dir_pose + str(('%04d'% i)) + '_pose.png', image_2d)"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('--txtdir',default='pose3dtxt/', help='save results to txt dir')
    parser.add_argument('--txtname', default='pose3d', help='save results to txt')
    ...
    output_dir = './runs/' + video_name + '/'

    get_pose2D(video_path, output_dir)
    get_pose3D(video_path, output_dir, args ,video_name)
    img2video(video_path, output_dir)
    print('Generating demo successful!')
    
    elapsed_time = time.time() - start_timer  #プログラム実行時間
    print("\n実行時間")
